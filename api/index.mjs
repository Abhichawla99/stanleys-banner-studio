...
// On Vercel: /tmp for writes; templates are read from the committed repo (process.cwd()/templates)
// Local (api/ folder): project root is one level up
const WORK_DIR      = IS_VERCEL ? "/tmp" : path.resolve(__dirname, "..");
const TEMPLATES_DIR = IS_VERCEL
  ? path.resolve(process.cwd(), "templates")   // Vercel sets cwd to project root
